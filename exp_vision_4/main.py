...
def main(args):
    ...
    #torch.save(model.state_dict(), "models/" + ("all" if train_config.all_classes else "binary"))
    ...
    print(model_config)
    inp, proc, mask, label, pos_label = next(iter(shapley_loader))
    print(inp.shape, proc.shape, mask.shape, label.shape, pos_label.shape)
    true_labels = [d[-2] for d in iter(shapley_loader)]
    ...
    save_matrices([*zip(*outputs), true_labels], output_path)
